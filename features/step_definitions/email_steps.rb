...
module EmailHelpers
  def current_email_address
    ...
    last_email_address || "example@kinstantly.com"
  end
end
...
Then /^(?:I|they|"([^"]*?)") should receive (a|an|no|\d+) welcome emails?$/ do |address, amount|
  ...
  expect(unread_emails_for(address).select { |m| m.subject =~ Regexp.new(Regexp.escape(I18n.t('devise.mailer.on_create_welcome.subject'))) }.size).to eq parse_email_count(amount)
end
