...
describe StripeChargeMailer, type: :mailer, payments: true do
	...
	context "customer charge notification email" do
		...
		it "should be delivered to the customer that was charged" do
			expect(email).to deliver_to customer_email
		end
		...
		it "should show the charge amount" do
			expect(email).to have_body_text display_currency_amount new_stripe_charge.amount_usd
		end
		
		it "should have a link to charge details" do
			expect(email).to have_body_text show_to_client_stripe_charge_url new_stripe_charge
		end
		...
		it "should show the card description" do
			expect(email).to have_body_text display_payment_card_in_email new_stripe_charge.stripe_card
		end
		
		it "should show the charge description" do
			expect(email).to have_body_text new_stripe_charge.description
		end
		
		it "should show the statement description" do
			expect(email).to have_body_text new_stripe_charge.statement_description
		end
	end
	...
	context "customer refund notification email" do
		...
		it "should show the refund amount" do
			...
			expect(email).to have_body_text Regexp.new('refunded\s+' + Regexp.escape(refunded_amount))
		end
		
		it "should have a link to refund details" do
			expect(email).to have_body_text show_to_client_stripe_charge_url refunded_stripe_charge
		end
		...
		it "should show the card description" do
			expect(email).to have_body_text display_payment_card_in_email refunded_stripe_charge.stripe_card
		end
		
		it "should show the original charge description" do
			expect(email).to have_body_text refunded_stripe_charge.description
		end
		
		it "should show the statement description of the original charge" do
			expect(email).to have_body_text refunded_stripe_charge.statement_description
		end
	end
