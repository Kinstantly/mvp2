...
We are using Ruby 2.3.0p0 and Rails 3.2.22.2 (as of March 30, 2016).  The Gemfile directs Heroku to use Ruby 2.3.0 and Rails 3.2.22.2.

The Rails maintainers test version 3.2.22 against Ruby 2.2.2 (the last time I checked) and there is no guarantee that Rails 3.2.22 will work with later versions of Ruby.  However, our application appears to be working fine with Ruby 2.3.0.  I am currently upgrading to the latest stable version of Rails 4.

For Ruby you should use at least ruby 2.2.2p95 to include the latest security updates.  You can install or upgrade using RVM.
...
If you're using RVM, it will parse the Ruby version from the Gemfile as long as you don't have other RVM configuration files overriding the Gemfile.  To ensure that you'll always use the correct version in this repository, you can do this.
 ...
 rvm --ruby-version use 2.3.0 # Or whatever the correct version number is.

Get the latest version of RubyGems (for security reasons).

 gem update --system
 gem --version # Should be at least version 2.6.4.

Install Bundler.

 gem install bundler
 bundle --version # Must be at least version 1.12.1 to keep Gemfile.lock compatible across developers.
...
I'm running PostgreSQL 9.5.2 for my development instance.  Installation (on Mac OS X) and aliases:
...
Note: To test production precompilation in your development environment, you might have to temporarily append the following to config/database.yml. *Be sure to remove this temporary configuration from the file, otherwise you will mess up the production environment!!*

 production:
   adapter: postgresql
   encoding: unicode
   database: does_not_exist_dbname
   pool: 5
