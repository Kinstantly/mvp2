...
We are using Ruby 2.0.0 and Rails 3.2.21 (as of Feb. 20, 2015).  The Gemfile directs Heroku to use Ruby 2.0.0.

The Rails maintainers test version 3.2.21 against Ruby 2.0.0 and there is no guarantee that Rails 3.2.21 will work with later versions of Ruby.  I know for a fact that Rails 3.2.21 does not work with Ruby 2.2.0.  Our plan is first to upgrade to the latest stable version of Rails 4 and then upgrade to the latest stable version of Ruby that supports that Rails version.

For Ruby you should use at least patch level 598, i.e., ruby 2.0.0p598, to include the latest security updates.  You can install or upgrade using RVM.
...
If you're using RVM, it will parse the Ruby version from the Gemfile as long as you don't have other RVM configuration files overriding the Gemfile.  To ensure that you'll always use the correct version in this repository, you can do this.
 ...
 rvm --ruby-version use 2.0.0 # Or whatever the correct version number is.
...
I'm running PostgreSQL 9.4.0 for my development instance.  Installation (on Mac OS X) and aliases:
...
I'm using Unicorn to start the web process(es).  In development I usually run one web process.  The default is to run three.  To run just one web process, set the WEB_CONCURRENCY variable.
...
To fix this, make sure you're using at least Ruby 2.0.0.  If you've installed Ruby via RVM, then the following edit should fix the problem.
