...
For Ruby you should use at least patch level 484, i.e., ruby 1.9.3p484, to include the latest security updates.  You can install or upgrade using RVM.
...
We have configured the application to use caching in production mode, e.g., for fragment caching.  You'll want to make sure that changes to view templates using fragment caching and models that are used in key-based caching will work OK with caching in production.  You can check this by turning on caching in development mode.  We are using memchached for caching, so you'll need to install it in your development environment if you don't already have it.

 # On MacOS.
 brew install memchached
 # Useful aliases.
...
See the {README_TESTING.rdoc}[rdoc-ref:README_TESTING] file.
