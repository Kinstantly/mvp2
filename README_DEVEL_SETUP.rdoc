...
We are using Ruby 2.2.2 and Rails 3.2.22 (as of July 9, 2015).  The Gemfile directs Heroku to use Ruby 2.2.2.

The Rails maintainers test version 3.2.22 against Ruby 2.2.2 and there is no guarantee that Rails 3.2.22 will work with later versions of Ruby.  Our plan is to upgrade to the latest stable version of Rails 4.

For Ruby you should use at least patch level 95, i.e., ruby 2.2.2p95, to include the latest security updates.  You can install or upgrade using RVM.
...
If you're using RVM, it will parse the Ruby version from the Gemfile as long as you don't have other RVM configuration files overriding the Gemfile.  To ensure that you'll always use the correct version in this repository, you can do this.
 ...
 rvm --ruby-version use 2.2.2 # Or whatever the correct version number is.

Get the latest version of RubyGems (for security reasons).

 gem update --system
 gem --version # Should be at least version 2.4.8.

Install Bundler.

 gem install bundler
 bundle --version # Must be at least version 1.10.5 to keep Gemfile.lock compatible across developers.
