...
If you ran 'bundle exec rake db:create', then you should already have a mvp2_performance database.  But if you don't have one, run the following command.
...
To be safe, modify all of the user email addresses and passwords in the performance database using the Rails console.
  ...
  User.all.each{ |u| u.update_column(:email, "example+perf#{u.id}@example.com"); u.update_column(:encrypted_password, '*'); u.password = u.password_confirmation = 'ffffff'; u.save }
  exit
