...
The '.rspec' file contains the '--drb' switch, which tells Rspec to use Spork if it is available.  So you don't need to specify it on the command line.  For example, to run all of the specs use this command:

 bundle exec rspec spec

To use Spork with Cucumber, you need to specify the '--drb' switch.  For example, to run all of the features use this command:
...
Here are some useful aliases for starting and monitoring the Rspec and Cucumber Spork servers.  You need to use these aliases from the repository root.
 ...
 alias sporkrspeclog='less log/sporkrspec.log'
 alias sporkcukestart='bundle exec spork cucumber >> log/sporkcuke.log 2>&1 &'
 alias sporkcukelog='less log/sporkcuke.log'
